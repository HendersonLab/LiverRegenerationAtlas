##-----------------------------------------------------------------------
# Perform data integration on Endothelia after cleansing (2nd iteration on dataset)
##-----------------------------------------------------------------------
...
io$out_dir <- paste0(results_dir, opts$lineage_anno, "/01_integrate/endo_project/")
...
seu_anno <- read.csv(
  ...
  dplyr::select(anno_broad, anno) %>%
  dplyr::mutate_at(c("anno_broad", "anno"), factor)
# Add metadata information to current Seurat object and filter
seu <- Seurat::AddMetaData(seu, metadata = seu_anno)
Idents(object = seu) <- "anno"
seu <- subset(seu, idents = c("MZ_LSEC1", "MZ_LSEC2", "CV_EC1", "PP_LSEC", "MitoHigh", "Injury_EC1",
                              "Portal_EC", "Central_EC", "Lymphatic_EC", "Cycling",
                              "Injury_EC2", "HepArterty_EC"), invert = FALSE)
...
opts$res <- seq(from = 0.7, to = 0.7, by = 0.1)
