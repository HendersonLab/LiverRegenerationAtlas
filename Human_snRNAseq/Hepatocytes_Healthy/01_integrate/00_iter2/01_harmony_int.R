##-----------------------------------------------------------------------
# Perform data integration on Healthy Hepatocytes after whole
# lineage integration (2nd iteration on dataset)
##-----------------------------------------------------------------------
...
iter_dims <- 40
iter_res <- 0.6
iter_dir <- paste0(results_dir, lineage_anno, "_",
                   condition, "/01_integrate/00_iter1/")
...
io$out_dir <- paste0(results_dir, opts$lineage_anno, "_",
                     opts$condition, "/01_integrate/00_iter2/")
...
seu <- subset(seu, idents = c("Hepatocytes", "MitoHigh"), invert = FALSE)
...
opts$meta_to_plot <- c("sample", "doublet_prediction")
...
opts$modules_group <- list(lineage = human_liver_lineage_markers,
                           ...
                           hep_manual_zonation = hep_manual_zonation_markers,
