...
# lineage integration (1st iteration on dataset)
##-----------------------------------------------------------------------
...
iter_dims <- 80
iter_res <- 0.3
iter_dir <- paste0(results_dir, "/01_integrate/00_iter1/")
...
Idents(object = seu) <- "condition"
seu <- subset(seu, idents = c(opts$condition), invert = FALSE)
