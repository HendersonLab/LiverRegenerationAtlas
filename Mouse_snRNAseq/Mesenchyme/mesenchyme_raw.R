...
ml_apap_mes <- ml_apap$seu
ml_apap_mes$annotation <- annotation$anno
ml_apap_mes <- SetIdent(ml_apap_mes, value="annotation")
ml_apap_mes <- subset(ml_apap_mes, idents=c("Mesenchyme"))
rm(ml_apap)
rm(annotation)
...
ml_apap_mes <- run_harmony_pipeline(
  seu_obj = ml_apap_mes,
  out_dir = "/Analysis/Mouse_Nuclei_apap/mesenchyme_raw/",
  batch_id = "sample",
