##-----------------------------------------------------------------------
# Perform data integration on all cells after QC (1st iteration on dataset)
##-----------------------------------------------------------------------
...
# Input/Output
io <- list()
io$out_dir <- paste0(results_dir, "/01_integrate/00_iter1/")
...
opts$res <- seq(from = 0.1, to = 0.5, by = 0.1)
...
opts$metadata_to_plot <- c("sample", "condition", "doublet_prediction")
...
opts$modules_group <- list(lineage = mouse_liver_lineage_markers,
                           hep_mouse_zonation = hep_mouse_zonation_markers,
                           hep_human_zonation = hep_human_zonation_markers,
                           hep_migratory_top25 = hep_migratory_livreg_top25_markers)
